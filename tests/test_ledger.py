...
def test_ledger_estimates_controlled_skill_lift(tmp_path: Path) -> None:
    ...
    assert stats.estimated_lift_standard_error == pytest.approx(0.1859)
    assert stats.estimated_lift_lower_bound == pytest.approx(0.1911)
    assert stats.estimated_lift_upper_bound == pytest.approx(0.92)
